...
function OceananigansSimulation(
    ...
    closure = if simple_ocean
        # ClimaOcean.simplified_ocean_closure() adds biharmonic + implicit vertical
        # mixing and exceeds the limit with tripolar active_cells_map 4.133KiB > 4.00KiB on P100.
        @info "Using simpler ocean setup; to be used for software testing only."
        (
            OC.HorizontalScalarDiffusivity(ν = 1e4),
            OC.ConvectiveAdjustmentVerticalDiffusivity(
                background_κz = 1e-5,
                convective_κz = 0.1,
                background_νz = 1e-4,
                convective_νz = 0.1,
