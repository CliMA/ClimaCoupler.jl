...
#
# VERDICT (2026-08-07, 6 iterations, driver exited 0).
# Residual trajectory (RMS sigma): lwp 0.65 -> 0.69, clt 0.76 -> 0.84,
# zonal swcre 0.90 -> 0.76. Posterior (constrained mean, displacement in
# prior sigma): eps_rel 0.048 (+0.67), margin 0.946 (+0.03), abs_margin
# 1.061 (+0.11), sharpness 0.802 (-0.23), residual 0.0456 (+0.23),
# Nd 7.08e7 (-0.13).
# P1 FAIL: clt ended 0.08 sigma above its start, and iterations 1 and 6
#    grade the same September (2006), so this is a same-weather
#    comparison. The hypothesis is HALF right: release degraded clt by
#    0.17 at the same point, so deleting the pattern term removed about
#    half the clt cost and stopped the swcre overfit (0.76 at floor vs
#    release below floor). The other half of the trade survived the
#    reduction: the zonal amplitude constraint is still paid with
#    cloud-field parameters (eps_rel +0.67, Nd flat). The optimizer
#    still sells clt for swcre, at half the price.
# P2 HALF: zonal swcre improved 0.90 -> 0.76, but not via Nd, which
#    barely moved (-0.13 vs release's -0.94). With the pattern term
#    deleted, the weaker amplitude-only pull was satisfied by the
#    cloud-fraction parameters alone; the optical knob was not needed.
# P3 PASS: eps_rel +0.67 sigma vs release's +1.17. Halving the swcre
#    constraint halved the eps_rel excursion again, confirming swcre
#    pattern chase as its driver.
# Conclusion: partial confirmation. The reduction halves the trade and
# is kept for the EDMF run, where frozen closure params close the
# residual eps_rel route. Watch the EDMF run for the trade reopening
# through the EDMF parameters (its P3): that outcome would say any
# swcre constraint next to clt funds cloud removal, and the fix is
# weight, not representation. The EDMF fixed TOML freezes this
# posterior (toml/calibration_edmf_fixed.toml).
#
