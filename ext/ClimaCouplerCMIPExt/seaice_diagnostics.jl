...
"""
function add_seaice_diagnostics!(ice_sim::ClimaSeaIceSimulation;
                                 output_dir = ".",
                                 surface_averaging_interval = 1days,
                                 checkpoint_interval = 90days,
                                 filename_prefix = "seaice",
    ...
    ice.output_writers[:surface_averages] = OC.JLD2Writer(ice.model, surface_outputs;
        schedule = OC.AveragedTimeInterval(surface_averaging_interval),
        filename = joinpath(output_dir, filename_prefix * "_surface"),
        file_splitting,
        overwrite_existing = true)

    ice.output_writers[:checkpointer] = OC.Checkpointer(ice.model;
