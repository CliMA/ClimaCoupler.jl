...
#
# Use: point ClimaCalibrate.add_workers at this script with
# `exename = <path>` (run_calibration.jl does this when
# CALIBRATION_WORKER_EXENAME is set). CALIBRATION_JULIA selects the Julia
# binary; it defaults to `julia` on PATH.
set -u
BIND=$(ip -o -4 addr show | awk '$2 ~ /^hsn/ {print $4; exit}' | cut -d/ -f1)
JULIA=${CALIBRATION_JULIA:-julia}
if [ -z "$BIND" ]; then
