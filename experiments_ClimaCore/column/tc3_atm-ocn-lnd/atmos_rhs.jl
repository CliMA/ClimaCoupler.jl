...
getindex(var::Fields.Field, idx ) = getindex(parent(var), idx)
...
function ∑tendencies_atm!(dY, Y, (parameters, T_sfc), t)
    ...
    surface_flux_ρθ = - calculate_sfc_fluxes_energy(DryBulkFormulaWithRadiation(), parameters, T_sfc[1], ρθ[1] / ρ[1] , u_1, v_1, ρ_1, t ) ./ C_p
    surface_flux_u =  - Cd * u_1 * sqrt(u_1^2 + v_1^2)
    surface_flux_v =  - Cd * v_1 * sqrt(u_1^2 + v_1^2)

    # accumulate in the required right units
    @inbounds begin
        dY.∫flux_u .= - ρ_1 * surface_flux_u .+ dY.∫flux_u .* 0.0  # 
        dY.∫flux_v .= - ρ_1 * surface_flux_v .+ dY.∫flux_u .* 0.0  # 
        dY.∫flux_h .= - C_p * surface_flux_ρθ.+ dY.∫flux_u .* 0.0 # W / m^2
    end
