...
function ClimaCalibrate.observation_map(iteration)
    ...
    preprocessed_vars = JLD2.load_object(
        joinpath(pkgdir(ClimaCoupler), "experiments/calibration/gravity_wave_daily/preprocessed_vars.jld2"),
    )
    ...
    for m in 1:n_members
        ...
        # Find the job output folder (named after config, e.g., "amip_land_daily")
        ...
        # Find the output subdirectory (could be "output_active" or "output_0000", etc.)
        output_subdirs = filter(d -> startswith(basename(d), "output"),
            [joinpath(job_dir, f) for f in readdir(job_dir) if isdir(joinpath(job_dir, f))])
        if isempty(output_subdirs)
            error("No output_* directory found in $job_dir")
        end
        output_dir_member = first(output_subdirs)
        # SimDir expects the clima_atmos subdirectory containing NetCDF files
        simdir_path = joinpath(output_dir_member, "clima_atmos")
...
function process_member_to_g_vector(diagnostics_folder_path, iteration)
    ...
    norm_path = joinpath(pkgdir(ClimaCoupler), "experiments/calibration/gravity_wave_daily/normalization.jld2")
    if !isfile(norm_path)
