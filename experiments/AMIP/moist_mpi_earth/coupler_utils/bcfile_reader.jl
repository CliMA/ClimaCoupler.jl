...
function bcfile_info_init(datafile_rll, varname, boundary_space; interpolate_monthly = false, segment_idx0 = [Int(1)], scaling_function = false)
    ...
    weightfile, datafile_cgll, regrid_space = ncreader_rll_to_cgll_from_space(FT, datafile_rll, varname, boundary_space, outfile = varname*"_cgll.g")
    ...
    if interpolate_monthly == true
        return BCFileInfo(datafile_cgll, varname, weightfile, regrid_space, Dates.Date.(ds["time"][:]), segment_idx0 .- Int(1), segment_idx0, current_fields, segment_length, scaling_function,[interpolate_monthly]) # TODO: generalize to DateTime (only dialy resol now)
    else
        return BCFileInfo(datafile_cgll, varname, weightfile, regrid_space, nothing, segment_idx0 .- Int(1), segment_idx0, current_fields, segment_length, scaling_function, [interpolate_monthly]) 
    end
end
