...
function setup_and_run(config_file = joinpath(pkgdir(ClimaCoupler), "config/ci_configs/amip_default.yml"))
    ...
    =#
    manual_restart = !isnothing(restart_t) && !isnothing(restart_dir)
    if manual_restart
        sim_was_restarted = Checkpointer.restart!(cs, restart_dir, restart_t)
    else
        sim_was_restarted = Checkpointer.auto_restart!(cs, dir_paths.checkpoints)
    end

    if !sim_was_restarted
        #=
