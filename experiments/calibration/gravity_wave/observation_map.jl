...
# ERA5 uses 1979-01-01 as the time epoch - all time values must be relative to this date
    ...
    if ClimaAnalysis.has_pressure(var)
        if ClimaAnalysis.has_time(var)
            ...
            if length(time_vals) == 1
                ...
                # Convert time from simulation epoch to ERA5 epoch (1979-01-01)
                # Simulation uses start_date from var.attributes, ERA5 uses 1979-01-01
                ...
                if !isnothing(sim_start_date_str)
                    ...
                    # time_val is seconds since sim_start_date
                    # Convert to seconds since ERA5_EPOCH
                    absolute_datetime = sim_start_date + Dates.Second(round(Int, time_val))
                    era5_time_val = Float64(Dates.value(absolute_datetime - ERA5_EPOCH) / 1000)  # milliseconds to seconds
                else
                    era5_time_val = time_val  # fallback if no start_date
                end
                ...
                # Update start_date to ERA5 epoch since we converted time values
...
function plot_bias(ekp, simdir, iteration; output_dir = simdir.simulation_path)
    ...
    for short_name in CALIBRATE_CONFIG.short_names
        ...
        # Note: Skip date shifting - just use the data directly for bias comparison
        var = set_units(var, var_units[short_name])
        # Only window if we have a proper date range (not single point)
        if sample_date_ranges[1] != sample_date_ranges[2]
        ...
        if ClimaAnalysis.has_pressure(var)
            if ClimaAnalysis.has_time(var)
                time_vals = ClimaAnalysis.times(var)
                # Use last time point for bias comparison
                if length(time_vals) >= 1
                    time_val = time_vals[end]
                    var_3d = ClimaAnalysis.slice(var; time = time_val)
                    lon = ClimaAnalysis.longitudes(var_3d)
                    lat = ClimaAnalysis.latitudes(var_3d)
                    var_resampled = ClimaAnalysis.resampled_as(var_3d; lon, lat, pressure_level = ERA5_PRESSURE_LEVELS)
