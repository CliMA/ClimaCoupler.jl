...
const config_file = joinpath(pkgdir(ClimaCoupler), "experiments", "calibration", "short_term", "model_config.yml")
...
function ClimaCalibrate.forward_model(iter, member)
    ...
    spinup_days = 92
    nweeks = length(minibatch)
    t_end_days = spinup_days + 7 * nweeks
