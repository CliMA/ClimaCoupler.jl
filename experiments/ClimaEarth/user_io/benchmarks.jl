...
parsed_args = Postprocessor.get_benchmark_args()
...
# Set up a dictionary mapping run names to descriptions
run_names_to_descriptions = Dict(
...
for (run_name, description) in run_names_to_descriptions
    run_info = Postprocessor.get_run_info(parsed_args, run_name)
    data = Postprocessor.append_table_data(data, description, run_info...)
end
