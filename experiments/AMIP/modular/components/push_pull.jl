...
#     parent(ice_sim.integrator.p.F_rad) .= parent(Regridder.binary_mask.(ice_fraction, threshold = eps()) .* csf.F_R_sfc)
...
#     # set atmos boundary conditions
#     ρ_1 = Spaces.level(Y.c.ρ , 1)

#     @. atmos_sim.integrator.p.dif_flux_energy_bc = - Geometry.WVector(F_shf + F_lhf) # Geometry.WVector(sfc_conditions[colidx].F_shf + sfc_conditions[colidx].F_lhf,)
#     @. atmos_sim.integrator.p.dif_flux_ρq_tot_bc = - Geometry.WVector(EVAP)
#     @. atmos_sim.integrator.p.dif_flux_uₕ_bc = - Geometry.Contravariant3Vector(atmos_sim.integrator.p.surface_normal[colidx]) ⊗ Geometry.Covariant12Vector(Geometry.UVVector(τxz / ρ_1, τyz / ρ_1),)
