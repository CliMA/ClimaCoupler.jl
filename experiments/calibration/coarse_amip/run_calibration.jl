...
if abspath(PROGRAM_FILE) == @__FILE__
    priors = [
        PD.constrained_gaussian("entr_inv_tau", 0.4, 0.25, 0, 1),
    ]
    prior = EKP.combine_distributions(priors)

    observation_vector =
        JLD2.load_object(joinpath(pkgdir(ClimaCoupler),"experiments/calibration/obs_vec.jld2"))
    ...
    verbose = true
    backend = ClimaCalibrate.ClimaGPUBackend(ClimaCalibrate.project_dir(), model_interface, hpc_kwargs, verbose)

    eki = ClimaCalibrate.calibrate(backend,
        ekp,
