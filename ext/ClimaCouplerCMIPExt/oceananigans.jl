...
function ocean_flux_highlat_mask(grid; location = (OC.Center(), OC.Center(), OC.Center()))
    polar_flux_lat_deg = 80.0  # zero fluxes where |lat| ≥ this (same band as polar_mask)
...
function FluxCalculator.update_turbulent_fluxes!(sim::OceananigansSimulation, fields)
    ...
    # Weight by (1 - sea ice concentration) and zero polar buffer
    ice_concentration = OC.interior(ice_concentration_field, :, :, 1)
    # polar-exclusion mask
    OC.interior(F_turb_ρτxz_cell, :, :, 1) .=
        polar_excl_centers .* OC.interior(F_turb_ρτxz_cell, :, :, 1) .*
        (1.0 .- ice_concentration) ./ reference_density
    OC.interior(F_turb_ρτyz_cell, :, :, 1) .=
        polar_excl_centers .* OC.interior(F_turb_ρτyz_cell, :, :, 1) .*
        (1.0 .- ice_concentration) ./ reference_density
...
function FieldExchanger.update_sim!(sim::OceananigansSimulation, csf)
    ...
    # apply polar-exclusion mask
    rad_T_flux = sim.remapping.scratch_arr3
    rad_T_flux .=
        polar_excl_centers .* (1.0 .- ice_concentration) .* (
            -(1 - α) .* remapped_SW_d .-
            ϵ * (
                remapped_LW_d .-
