...
"""
function load_var(filepath, short_name; varname=nothing, flip_sign=false, transform_dates=nothing, )
    # TODO: Don't assume monthly data
    var = isnothing(varname) ? OutputVar(filepath) : OutputVar(filepath, varname; shift_by = Dates.firstdayofmonth)
    ...
    if !isnothing(transform_dates)
        var = ClimaAnalysis.Var.transform_dates(var, date -> date - transform_dates)
    end
...
function load_vars()
    ...
    flux_file = joinpath(@clima_artifact("era5_monthly_averages_surface_single_level_1979_2024"), "era5_monthly_averages_surface_single_level_197901-202410.nc")
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    JLD2.save_object(
        joinpath(pkgdir(ClimaCoupler),"experiments/calibration/preprocessed_vars.jld2"),
        preprocessed_vars,
    )
    observation_vector = make_observation_vector(preprocessed_vars, sample_date_ranges)
    JLD2.save_object(
        joinpath(pkgdir(ClimaCoupler),"experiments/calibration/obs_vec.jld2"),
        observation_vector,
    )
end
