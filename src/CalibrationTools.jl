...
for short_name in (:rsdt, :rsut, :rlut, :rsutcs, :rlutcs, :rsds, :rsus, :rlds, :rlus, :rsdscs, :rsuscs, :rldscs)
    @eval preprocess(::CERESDataLoader, var, ::Val{$(QuoteNode(short_name))}) =
        _preprocess_ceres_var(var)
end

function _preprocess_ceres_var(var)
    short_name = ClimaAnalysis.short_name(var)

    var_units = ClimaAnalysis.units(var)
    var_units in keys(CERES_TO_CLIMA_UNITS) &&
        (var = ClimaAnalysis.set_units(var, CERES_TO_CLIMA_UNITS[var_units]))

    ClimaAnalysis.set_short_name!(var, short_name)
    return var
end

function Base.show(io::IO, data_loader::CERESDataLoader)
    vars = sort(collect(data_loader.available_vars))
    printstyled(io, "CERESDataLoader", bold = true, color = :green)
    print(io, ": ")
    print(io, join(vars, ", "))
end
