...
function FluxCalculator.water_albedo_from_atmosphere!(
    ...
    direct_albedo .=
        CA.surface_albedo_direct(
            α_model,
        ).(λ, μ, Interfacer.remap(LinearAlgebra.norm.(CC.Fields.level(Y.c.uₕ, 1)), axes(direct_albedo)))
    diffuse_albedo .=
        CA.surface_albedo_diffuse(
            α_model,
        ).(λ, μ, Interfacer.remap(LinearAlgebra.norm.(CC.Fields.level(Y.c.uₕ, 1)), axes(diffuse_albedo)))
