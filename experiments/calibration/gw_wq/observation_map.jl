...
function ClimaCalibrate.observation_map(iteration)
    ...
    preprocessed_vars = JLD2.load_object(
        joinpath(pkgdir(ClimaCoupler), "experiments/calibration/gw_wq/preprocessed_vars.jld2"),
    )
    
    # Load land mask and land_only_vars info
    land_mask_path = joinpath(pkgdir(ClimaCoupler), "experiments/calibration/gw_wq/land_mask.jld2")
    land_mask = nothing
...
function process_member_to_g_vector(diagnostics_folder_path, iteration)
    ...
    norm_path = joinpath(pkgdir(ClimaCoupler), "experiments/calibration/gw_wq/normalization.jld2")
    if !isfile(norm_path)
