...
function compute_gradient_flux!(
    ...
    κʰ, κʰ, κᶻ = bl.bl_prop.get_diff()
    νʰ, νʰ, νᶻ = bl.bl_prop.get_visc()

    F_θ =
        bl.bl_prop.calc_diff_flux(G.∇θ, A.npt, A.elnum, A.xc, A.yc, A.zc, κʰ, κᶻ)
    GF.κ∇θ = F_θ

    F_u =
        bl.bl_prop.calc_visc_flux(G.∇u, A.npt, A.elnum, A.xc, A.yc, A.zc, νʰ, νᶻ)
    GF.ν∇u = F_u
...
"""
  ...
  - calc_visc_flux  :: function to set calculation of viscous fluxes
...
function prop_defaults()
    ...
    get_diff(_...) = (return 0.0, 0.0, 0.0)
    bl_prop = (bl_prop..., get_diff = get_diff)

    get_visc(_...) = (return 0.0, 0.0, 0.0)
    bl_prop = (bl_prop..., get_visc = get_visc)

    calc_diff_flux(_...) = (return 0.0, 0.0, 0.0)
    bl_prop = (bl_prop..., calc_diff_flux = calc_diff_flux)

    calc_visc_flux(_...) = (return 0.0, 0.0, 0.0)
    bl_prop = (bl_prop..., calc_visc_flux = calc_visc_flux)
