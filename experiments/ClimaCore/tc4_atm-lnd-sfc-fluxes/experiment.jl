...
function postprocess(sol_atm, sol_lnd; visualize = false )
    ...
    static_energy_ = [sum( static_energy(parent(u.x[1])[:,4], parent(u.x[1])[:,1], parent(z_c), parameters_.C_p, parameters_.R_d, parameters_.grav, parameters_.MSLP) + kinetic_energy.(parent(u.x[1])[:,2], parent(u.x[1])[:,3]) .* 0.0) for u in sol_atm.u]
