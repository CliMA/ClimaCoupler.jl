...
totalsteps = 20 # total simulation coupled cycle steps
...
function main(::Type{FT}) where {FT}
    ...
    atmos_calc_diff_flux(∇θ, npt, el, x, y, z, κᵃʰ, κᵃᶻ) = - Diagonal(@SVector([κᵃʰ, κᵃʰ, κᵃᶻ])) * ∇θ  
    atmos_calc_visc_flux(∇u, npt, el, x, y, z, νᵃʰ, νᵃᶻ) = - Diagonal(@SVector([νᵃʰ, νᵃʰ, νᵃᶻ])) * ∇u              # Set atmos diffusion coeffs
    ...
    ocean_calc_diff_flux(∇θ, npt, el, x, y, z, κᵒʰ, κᵒᶻ) = - Diagonal(@SVector([κᵒʰ, κᵒʰ, κᵒᶻ])) * ∇θ              # Set ocean diffusion coeffs
    ocean_calc_visc_flux(∇u, npt, el, x, y, z, νᵒʰ, νᵒᶻ) = - Diagonal(@SVector[νᵒʰ, νᵒʰ, νᵒᶻ] ) * ∇u   
    ...
    simulation = (;
        ...
        name = "Coupler_BoxUnitTest_VariableAdvection_ctrl",
        )
