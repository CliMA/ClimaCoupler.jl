...
function CoupledSimulation(config_dict::AbstractDict)
    ...
    if sim_mode <: AMIPMode || sim_mode <: CMIPMode || sim_mode <: SubseasonalMode
        ...
        if sim_mode <: SubseasonalMode
            isnothing(era5_initial_condition_dir) && error("subseasonal mode requires --era5_initial_condition_dir")
            # Filenames inferred from start_date, which is YYYYMMDD
            datestr = Dates.format(start_date, Dates.dateformat"yyyymmdd")
            subseasonal_sst = joinpath(era5_initial_condition_dir, "sst_processed_$(datestr)_0000.nc")
            subseasonal_sic = joinpath(era5_initial_condition_dir, "sic_processed_$(datestr)_0000.nc")
            subseasonal_land_ic = joinpath(era5_initial_condition_dir, "era5_land_processed_$(datestr)_0000.nc")
        end
