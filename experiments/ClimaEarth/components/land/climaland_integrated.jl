...
function FluxCalculator.compute_surface_fluxes!(
    ...
    # We use parent here because of the mismatch between the lowest atmos level
    ...
    parent(coupled_atmos.u) .= parent(uₕ_int)
    parent(coupled_atmos.thermal_state) .= parent(thermo_state_int)
    parent(coupled_atmos.h) .= parent(z_int)
    ...
    fluxes = coupler_land_turbulent_fluxes!(p, sim.model, Y, t, coupled_atmos)
...
"""
    coupler_land_turbulent_fluxes(model, Y, p, t, coupled_atmos)
...
It also takes in `coupled_atmos`, a ClimaLand.CoupledAtmosphere containing the
atmospheric properties required for the flux calculation: `u`, `h`,
`thermal_state`, and `gustiness`.
...
"""
function coupler_land_turbulent_fluxes!(p, model::ClimaLand.LandModel, Y, t, coupled_atmos::ClimaLand.CoupledAtmosphere)
