...
function setup_model(FT, start_date, stop_date, Δt, domain, toml_dict)
    ...
    if LONGER_RUN
        ...
        era5_ncdata_path = ClimaLand.Artifacts.era5_land_forcing_data2008_path(; context)
    end
...
function ClimaCalibrate.forward_model(iteration, member)
    ...
    outdir = ClimaUtilities.OutputPathGenerator.generate_output_path(diagnostics_dir)

    Δt = 450.0

    domain =
        ClimaLand.Domains.global_domain(FT; context, nelements, mask_threshold = FT(0.99))

    calibrate_params_path = ClimaCalibrate.parameter_path(output_dir, iteration, member)
    default_params_filepath = joinpath(pkgdir(ClimaLand), "toml", "default_parameters.toml")
