...
"""
    flatten_var_with_latitude_weights(var; land_mask=nothing)
...
function make_observation_vector(vars_by_date, sample_date_ranges, short_names)
    ...
    JLD2.save_object(
        joinpath(pkgdir(ClimaCoupler), "experiments/calibration/gw_wq/land_mask.jld2"),
    ...
    JLD2.save_object(
        joinpath(pkgdir(ClimaCoupler), "experiments/calibration/gw_wq/normalization.jld2"),
        var_normalization
    )
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    JLD2.save_object(
        joinpath(
            pkgdir(ClimaCoupler),
            "experiments/calibration/gw_wq/preprocessed_vars.jld2",
        ),
        preprocessed_vars,
    )
