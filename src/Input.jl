...
This module handles parsing of configuration files, command-line arguments, and
other user inputs for setting up coupled simulations.
"""
module Input
...
export argparse_settings, parse_commandline, get_coupler_config_dict, get_coupler_args, parse_component_dts!

# Mode name dictionary for converting strings to types
...
Set up command-line argument parsing settings for ClimaCoupler.
"""
...
Parse command-line arguments using the provided ArgParse settings.
This is intended to be used with the `argparse_settings` function, e.g:
`parsed_args = Input. parse_commandline(Input.argparse_settings())`.
...
The order of priority for overwriting configuration options from lowest to highest is:
    ...
    2. ClimaCoupler defaults (defined in Input module)
