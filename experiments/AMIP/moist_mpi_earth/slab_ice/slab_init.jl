...
function solve_ice!(integ, Δt)
    ...
    sm = semster_zero_layer_model.(T_ml, T_sfc, h_ice, F_atm, ∂F_atm∂T_sfc, Ref(p), Ref(Δt))
...
function slab_ice_init(
    ...
    ice_mask = prescribed_sic_data !== nothing ? get_ice_mask.(prescribed_sic_data .- FT(25)) : ClimaCore.Fields.zeros(space) # here 25% and lower is considered ice free # TODO: generalize to a smaoot function of ice fraction
