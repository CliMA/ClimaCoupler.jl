...
function make_observation_vector(vars_by_date, sample_date_ranges, short_names)
    ...
    JLD2.save_object(
        joinpath(pkgdir(ClimaCoupler), "experiments/calibration/gravity_wave_daily/normalization.jld2"),
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    JLD2.save_object(
        joinpath(
            pkgdir(ClimaCoupler),
            "experiments/calibration/gravity_wave_daily/preprocessed_vars.jld2",
        ),
        preprocessed_vars,
    )
