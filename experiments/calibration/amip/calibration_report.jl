...
function calibration_report(
    ...
    p(
        ...
        _sig(ebest),
        "  (best is ",
        @sprintf("%.0f%%", 100 * (e1 - ebest) / e1),
        " below initial).",
    )
    p(
        "- Ensemble spread: contracted to ",
        @sprintf("%.0f%%", 100 * spread_ratio[end]),
...
function _observation_analysis(output_dir)
    ...
    # A dense covariance (correlated noise floor, see correlated_noise.jl) has
    # the floor folded into its off-diagonal structure, so only the total σ is
    # recoverable there.
    # One covariance block per OBS_NOISE_GROUPS entry, concatenated in group
    # order to match the flattened observation. Taking only covs[1] gives a
    # sigma vector as long as the first block while the per-variable ranges
    # span every block, which threw a BoundsError on every multi-group run.
    σtot = Float64[]
    σfloor_parts = Vector{Float64}[]
    all_svd = true
    S = nothing
    rank = nothing
    for cov in o1.covs
        if cov isa EKP.SVDplusD
            Sb = cov.svd_cov.S
            U = cov.svd_cov.U
            Vt = cov.svd_cov.Vt
            # EKP.SVDplusD does not implement size(A, i); the diagonal floor
            # has one entry per observation row.
            n = length(cov.diag_cov.diag)
            dlow = zeros(n)
            for k in 1:length(Sb)
                dlow .+= Sb[k] .* (U[:, k] .* Vt[k, :])
            end
            dfloor = cov.diag_cov.diag
            append!(σtot, sqrt.(max.(dlow .+ dfloor, 0.0)))
            push!(σfloor_parts, sqrt.(max.(dfloor, 0.0)))
            # Rank and singular values are per block; report the first block's.
            isnothing(S) && (S = Sb; rank = length(Sb))
        else
            append!(σtot, sqrt.(max.([cov[i, i] for i in 1:size(cov, 1)], 0.0)))
            all_svd = false
        end
    end
    σfloor = all_svd && !isempty(σfloor_parts) ? reduce(vcat, σfloor_parts) : nothing
    all_svd || (S = nothing; rank = nothing)
    ...
    pervar =
        ok ?
        [
            (name = names[i], v = vars[i], n = finite[i], rng = (offs[i] + 1):offs[i + 1])
            for i in 1:length(vars)
        ] : nothing

    return (; o1, y, vars, names, finite, pervar, σtot, σfloor, rank, S)
end
...
function _reduction_section(io, oa, cfg)
    ...
    p()
    zonal =
        all(v -> ClimaAnalysis.has_latitude(v) && !ClimaAnalysis.has_longitude(v), oa.vars)
    if zonal
        p(
            "- **Spatial: zonal (longitude) mean** — the longitude dimension is ",
    ...
    if zonal && !isnothing(cfg) && !isnothing(cfg.nlon)
        ...
        full =
            ...
            nlat *
            sum(
                v -> ClimaAnalysis.has_altitude(v) ? length(ClimaAnalysis.altitudes(v)) : 1,
                oa.vars,
            )
        p(
            "- The model grid has ~",
            cfg.nlon,
            " longitudes per latitude, so the ",
            "zonal mean collapses each latitude's ",
            cfg.nlon,
            " values to one. Combined with level selection this is ",
            length(oa.y),
            " points — roughly **",
            @sprintf("%d×", round(Int, full / length(oa.y))),
            " fewer** than the full lon×lat grid (~",
            full,
            ").",
        )
        p(
            "- Why it matters: the SVDplusD covariance only softens a handful of ",
...
function _covariance_section(io, oa)
    ...
    p()
    if isnothing(oa.rank)
        p(
            "- Type: **dense (correlated floor)** — a low-rank interannual ",
            "covariance plus a structural-error floor with a spatial ",
            "decorrelation length (see correlated_noise.jl).",
        )
    else
        p(
            "- Type: **SVDplusD** — a low-rank interannual covariance (estimated from the ",
    ...
    p()
    p(
        "- Interpretation: where the noise σ is comparable to or larger than the ",
...
function _fmt_ranges(ranges)
    ...
    parts = [
        first(r) == last(r) ? fmt(first(r)) : "$(fmt(first(r)))..$(fmt(last(r)))" for
        r in ranges
    ]
    return join(parts, ", ")
end
...
function _load_config_context(config_path)
    ...
    # A module built with Module() has no one-argument include, which configs
    # need to pull in prior_tools.jl.
...
function _narrative(
    ...
    io = IOBuffer()
    vars =
        !isnothing(obsnames) ? join(obsnames, "+") :
    ...
    print(
        io,
        "This calibration fit $(nens) ensemble members to $(obsdim) data points ",
        "of $(vars) (target: $(target)) over $(niter) iterations",
    )
    !isnothing(cfg) &&
        niter < cfg.n_iterations &&
        print(
            io,
            " (short of the $(cfg.n_iterations) targeted, typically because the workers hit their walltime)",
        )
    print(io, ". ")
    ...
    if collapsed
        print(
            io,
            "The ensemble spread ",
            one_step_crash ? "crashed after the first update" :
            "fell below 1% of its initial value",
        ...
        print(
            io,
            "The ensemble spread contracted gradually (to $(round(100*spread_ratio[end]))% of ",
...
if abspath(PROGRAM_FILE) == @__FILE__
    length(ARGS) >= 1 ||
        error("usage: calibration_report.jl <output_dir> [config_path] [label]")
    output_dir = ARGS[1]
