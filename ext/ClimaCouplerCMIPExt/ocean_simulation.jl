...
const TEOS10EquationOfState =
    OC.BuoyancyFormulations.SeawaterPolynomials.TEOS10.TEOS10EquationOfState
...
"""
default_or_override(default::Default, possibly_alternative_default = default.value) =
    possibly_alternative_default
...
Adapt.adapt_structure(to, bpf::BarotropicPotentialForcing) = BarotropicPotentialForcing(
    Adapt.adapt(to, bpf.direction),
    Adapt.adapt(to, bpf.potential),
)

const XDirectionBPF = BarotropicPotentialForcing{<:XDirection}
const YDirectionBPF = BarotropicPotentialForcing{<:YDirection}

@inline (bpf::XDirectionBPF)(i, j, k, grid, clock, fields) =
    -∂xᶠᶜᶜ(i, j, k, grid, bpf.potential)
@inline (bpf::YDirectionBPF)(i, j, k, grid, clock, fields) =
    -∂yᶜᶠᶜ(i, j, k, grid, bpf.potential)

#####
##### Utilities
#####

keep_user_boundary_condition(user, default) =
    user isa DefaultBoundaryCondition ? default : user
...
"""
function merge_boundary_conditions(
    user::OC.FieldBoundaryConditions,
    default::OC.FieldBoundaryConditions,
)
    return OC.FieldBoundaryConditions(
        keep_user_boundary_condition(user.west, default.west),
...
@inline spᶠᶜᶜ(i, j, k, grid, Φ) =
    @inbounds sqrt(Φ.u[i, j, k]^2 + ℑxyᶠᶜᵃ(i, j, k, grid, ϕ², Φ.v))
@inline spᶜᶠᶜ(i, j, k, grid, Φ) =
    @inbounds sqrt(Φ.v[i, j, k]^2 + ℑxyᶜᶠᵃ(i, j, k, grid, ϕ², Φ.u))

@inline u_quadratic_bottom_drag(i, j, grid, c, Φ, μ) =
    @inbounds -μ * Φ.u[i, j, 1] * spᶠᶜᶜ(i, j, 1, grid, Φ)
@inline v_quadratic_bottom_drag(i, j, grid, c, Φ, μ) =
    @inbounds -μ * Φ.v[i, j, 1] * spᶜᶠᶜ(i, j, 1, grid, Φ)

# Keep a constant linear drag parameter independent on vertical level
@inline u_immersed_bottom_drag(i, j, k, grid, clock, Φ, μ) =
    @inbounds -μ * Φ.u[i, j, k] * spᶠᶜᶜ(i, j, k, grid, Φ)
@inline v_immersed_bottom_drag(i, j, k, grid, clock, Φ, μ) =
...
const TripolarOfSomeKind = Union{
    OC.TripolarGrid,
    OC.ImmersedBoundaryGrid{<:Any, <:Any, <:Any, <:Any, <:OC.TripolarGrid},
}

function default_free_surface(
    grid::TripolarOfSomeKind;
...
default_stop_time(::AbstractArchitecture, clock) =
    clock.time isa Number ? Inf : Dates.DateTime(9999, 12, 31, 23, 59, 59)
default_stop_time(::ReactantState, clock) = nothing

hasclosure(closure, ClosureType) = closure isa ClosureType
hasclosure(closure_tuple::Tuple, ClosureType) =
    any(hasclosure(c, ClosureType) for c in closure_tuple)

const OceananigansModelSimulations = Union{
    OC.Simulation{<:OC.HydrostaticFreeSurfaceModel},
    OC.Simulation{<:OC.NonhydrostaticModel},
}
...
function ocean_simulation(
    ...
    if single_column_simulation
        ...
        u_immersed_drag = OC.FluxBoundaryCondition(
            u_immersed_bottom_drag,
            discrete_form = true,
            parameters = bottom_drag_coefficient,
        )
        v_immersed_drag = OC.FluxBoundaryCondition(
        ...
        u_immersed_bc =
            OC.ImmersedBoundaries.ImmersedBoundaryCondition(bottom = u_immersed_drag)
        v_immersed_bc =
            OC.ImmersedBoundaries.ImmersedBoundaryCondition(bottom = v_immersed_drag)
    ...
    u_bot_bc = OC.FluxBoundaryCondition(
        u_quadratic_bottom_drag,
        discrete_form = true,
        parameters = bottom_drag_coefficient,
    )
    v_bot_bc = OC.FluxBoundaryCondition(
    ...
    default_boundary_conditions = (
        u = OC.FieldBoundaryConditions(
            top = u_top_bc,
            bottom = u_bot_bc,
            immersed = u_immersed_bc,
        ),
        v = OC.FieldBoundaryConditions(
            top = v_top_bc,
            bottom = v_bot_bc,
    ...
    merged_boundary_conditions = NamedTuple(
        name =>
            haskey(default_boundary_conditions, name) ?
            merge_boundary_conditions(
                boundary_conditions[name],
                default_boundary_conditions[name],
            ) : boundary_conditions[name] for name in keys(boundary_conditions)
    )
