...
@inline νhb(i, j, k, grid, ℓx, ℓy, ℓz, clock, fields, λ) = OC.Operators.Az(i, j, k, grid, ℓx, ℓy, ℓz)^2 / λ
...
function default_ocean_closure(FT = OC.defaults.FloatType)
    ...
    return CATKEVerticalDiffusivity(OC.VerticallyImplicitTimeDiscretization(), FT; mixing_length, turbulent_kinetic_energy_equation)
end
...
function simplified_ocean_closure(FT = OC.defaults.FloatType)
    horizontal_viscosity = OC.HorizontalScalarBiharmonicDiffusivity(ν = νhb, discrete_form = true, parameters = 10days)
    convective_mixing = OC.ConvectiveAdjustmentVerticalDiffusivity(FT; convective_κz = 1.0, convective_νz = 1.0)
    vertical_mixing = OC.VerticalScalarDiffusivity(OC.VerticallyImplicitTimeDiscretization(), FT; ν = ν_step_simple, κ = κ_step_simple)
...
function default_one_degree_closure(; κ_skew = 500,
    ...
    horizontal_viscosity = OC.HorizontalScalarBiharmonicDiffusivity(ν = νhb, discrete_form = true, parameters = biharmonic_timescale)
