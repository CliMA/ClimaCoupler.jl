...
function largest_period(sample_date_range)
    ...
    day_in_ms = 8.64e7
    period = span.value >= day_in_ms * 365 ? Year(1) : 
...
function ClimaCalibrate.analyze_iteration(ekp, g_ensemble, prior, output_dir, iteration)
    ...
    plot_toa_flux_bias(simdir, iteration; output_dir = plot_output_path)
...
function plot_bias(ekp, simdir, iteration; output_dir = simdir.simulation_path)
    ...
    for (j, date) in enumerate(sample_dates)
        for (i, (sim_var, era5_var)) in enumerate(var_pairs)
            ...
            cmap_extrema = get(bias_plot_extrema, short_name(sim_var_t), extrema(sim_var_t.data))
            
            # Plot bias
            ax =ClimaAnalysis.Visualize.plot_bias_on_globe!(fig[i, j], sim_var_t, era5_var_t; cmap_extrema)
        end
    end
...
function plot_toa_flux_bias(simdir, iteration; output_dir = simdir.simulation_path)
    era5_toa_rad_fluxes_path = joinpath(pkgdir(ClimaCoupler), "era5_oct_2018_2024_toa_rad_fluxes.nc")
    sample_dates = CALIBRATE_CONFIG.sample_date_ranges[iteration + 1]
    short_names = ("rlut", "rsut",) #"rlutcs", "rsutcs"  )
    var_pairs = map(short_names) do short_name
        sim_var = get_var(short_name, simdir)
        sim_var = preprocess_var(sim_var, sample_dates)
        era5_var = if short_name == "rlut"
            -OutputVar(era5_toa_rad_fluxes_path, "avg_tnlwrf")
        elseif short_name == "rsut"
            era5_rsnt = OutputVar(era5_toa_rad_fluxes_path, "avg_tnswrf")
            era5_rsdt = OutputVar(era5_toa_rad_fluxes_path, "avg_tdswrf")
            era5_rsdt - era5_rsnt
        end
        era5_var.attributes["short_name"] = short_name
        era5_var = set_units(era5_var, "W m^-2")
        era5_var = reverse_dim(era5_var, latitude_name(era5_var))
        (sim_var, era5_var)
    end
    fig = GeoMakie.Figure(size = (1500, 500 * length(var_pairs)));
    for (i, (sim_var, era5_var)) in enumerate(var_pairs)
        for (j, date) in enumerate(unique(sample_dates))
            sim_var_t = slice(sim_var, time = date)
            era5_var_t = slice(era5_var, time = date)
            ax = ClimaAnalysis.Visualize.plot_bias_on_globe!(fig[i, j], sim_var_t, era5_var_t)
        end
    end
    GeoMakie.save(joinpath(output_dir, "toa_flux_bias.png"), fig)
    return nothing
end