...
#       experiments/calibration/amip/config/phase2.jl [--skip-wiring] [--sypd 2.0]
#
...
function parse_preflight_args(args)
    ...
    isempty(config_path) && error(
        "Usage: preflight.jl <config.jl> [--skip-wiring] [--sypd X]",
    )
...
function check_config(sypd)
    ...
    msg = "$(sim_days) sim days per member, " *
...
function atmos_params_from_toml(CA, base_tomls, member_toml, FT, gw_flags)
    override_file =
        CP.merge_toml_files(vcat(base_tomls, [member_toml]); override = true)
...
function check_wiring(CA, scratch)
    ...
    names = PD.get_name(Main.PRIORS)
    u0 = Statistics.mean(Main.PRIORS)
    su = sqrt.(Statistics.var(Main.PRIORS))
    center = PD.transform_unconstrained_to_constrained(Main.PRIORS, u0)

    unchecked = isdefined(Main, :PREFLIGHT_NON_ATMOS_PARAMS) ?
                Main.PREFLIGHT_NON_ATMOS_PARAMS : String[]
    ...
    for (i, name) in enumerate(names)
        ...
        u = copy(u0)
        u[i] += su[i]
        perturbed = PD.transform_unconstrained_to_constrained(Main.PRIORS, u)
        pert_toml =
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    include(
        joinpath(
            pkgdir(ClimaCoupler), "experiments", "calibration", "amip",
            "preprocessing.jl",
        ),
    )
