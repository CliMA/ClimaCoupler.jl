...
if prescribed_sic == true
    ...
    slab_ice_sim = slab_ice_init(
        ...
        saveat = saveat,
        prescribed_sic_data = SIC,
    )
else
    ocean_params = slab_ocean_sim.integrator.p.params
    slab_ice_sim = slab_ice_init(FT, tspan, ocean_params, dt = Δt_cpl, space = boundary_space, saveat = saveat)
end
...
# conservation  check
if @isdefined CSoffline
    check_conservation(CSoffline, coupler_sim, atmos_sim, slab_sim, slab_ocean_sim, slab_ice_sim, "conservation.png")
end

plot_global_energy(CS, coupler_sim, "total_energy_Xdyn.png")

# # animations
# include("coupler_utils/viz_explorer.jl")
# plot_anim(atmos_sim, slab_sim, slab_ice_sim, slab_ice_sim) # ice sim holds ocean info

# # vtk output for viz
# include("coupler_utils/vtk_helper.jl")
# space_new = retype_space(h_space, Float64)
# times = 0:saveat:t_end
# Sol_Tsfc = map(i -> convert_field_to_type(space_new, sol_slab.u[i].T_sfc), 1:length(times))
# Sol_Tsfc_ice = map(i -> convert_field_to_type(space_new, sol_slab_ice.u[i].T_sfc), 1:length(times))
# ClimaCoreVTK.writepvd(joinpath("data", "tt_5"), times, (T_sfc = Sol_Tsfc, T_sfc_ice = Sol_Tsfc_ice) , basis = :lagrange)
