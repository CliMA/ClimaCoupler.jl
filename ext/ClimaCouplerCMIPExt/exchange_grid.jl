...
`node_cov / node_cov_total` clamped to [0, 1] and made continuous with
`weighted_dss!`. The complement is the land fraction; sea ice and open ocean
partition the wet fraction itself (see `FieldExchanger.align_surface_fractions!`).
"""
function wet_ocean_fraction_field(
    ...
    # Reconcile shared element-boundary nodes.
