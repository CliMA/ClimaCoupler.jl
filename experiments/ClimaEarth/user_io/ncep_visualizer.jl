import Downloads
import NCDatasets
import ClimaCoupler: Diagnostics, PostProcessor
...
function ncep_paperplots(
    ...
    for vname in diags_vnames
        ...
        data, coords = Diagnostics.get_var(ncep_src, Val(vname))
        raw_tag = length(size(data)) == 3 ? PostProcessor.ZLatLonData() : PostProcessor.LatLonData()

        # postprocess
        post_data =
            PostProcessor.postprocess(vname, data, getproperty(post_spec, vname), coords = coords, raw_tag = raw_tag)

        # create individual plots
        zonal_mean_params = (; ylabel = "p (hPa)", yaxis = (;), yflip = true, getproperty(plot_spec, vname)...) # overwrite defaults
        p = Plots.plot(post_data, zmd_params = zonal_mean_params, hsd_params = (; getproperty(plot_spec, vname)...))
...
function download_read_nc(data_source::NCEPMonthlyDataSource, https::String, ncep_vname::String)
    ...
    NCDatasets.NCDataset(local_file) do ds
...
function Diagnostics.get_var(data_source::NCEPMonthlyDataSource, ::Val{:T})
...
function Diagnostics.get_var(data_source::NCEPMonthlyDataSource, ::Val{:u})
    https = "https://downloads.psl.noaa.gov/Datasets/ncep.reanalysis2/Monthlies/pressure/uwnd.mon.mean.nc"
    ncep_vname = "uwnd"
    download_read_nc(data_source, https, ncep_vname)
end

function Diagnostics.get_var(data_source::NCEPMonthlyDataSource, ::Val{:q_tot})
...
function Diagnostics.get_var(data_source::NCEPMonthlyDataSource, ::Val{:toa_fluxes})
...
function Diagnostics.get_var(data_source::NCEPMonthlyDataSource, ::Val{:precipitation_rate})
    https = "https://downloads.psl.noaa.gov/Datasets/ncep.reanalysis2/Monthlies/gaussian_grid/prate.sfc.mon.mean.nc"
    ncep_vname = "prate"
    download_read_nc(data_source, https, ncep_vname)
end

function Diagnostics.get_var(data_source::NCEPMonthlyDataSource, ::Val{:T_sfc})
