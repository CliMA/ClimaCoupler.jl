# 2-D preprocessing validation at 15 degrees: lwp+pr on a block-averaged
# longitude-latitude grid instead of the zonal mean.
#
# Same parameters, platform, and noise floors as the validated zonal lwp+pr
# calibration. The only change is the spatial reduction: COARSEN_FACTOR = 6
# block-averages the 2.5 degree comparison grid to 15 degrees (24 x 12), which
# is near the decorrelation scale of monthly means and marginally resolves the subtropical
# stratocumulus decks as 1 to 2 pixels.
#
...
# 2-D coarse observations for later phases. The companion config at 10 degrees
# (lwp_pr_2d10.jl) is preferred if both validate.
#
...
output_dir = joinpath(pkgdir(ClimaCoupler), "amip_calibration_lwp_pr_2d15")

const COARSEN_FACTOR = 6
