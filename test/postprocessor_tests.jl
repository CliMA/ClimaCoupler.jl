...
@testset "get_benchmark_args" begin
    ...
    try
        ...
        args = Postprocessor.get_benchmark_args()
        @test args["job_id_coupled"] == "test_coupled"
        @test isnothing(args["job_id_atmos"])
        @test args["coupler_output_dir"] == joinpath("experiments", "ClimaEarth", "output")
    finally
        empty!(ARGS)
...
@testset "get_run_info" begin
    ...
    for (run_type, expected_job_id) in [
        ...
        job_id, artifacts_dir = Postprocessor.get_run_info(parsed_args, run_type)
        @test job_id == expected_job_id
        @test artifacts_dir == joinpath(test_output_dir, expected_job_id, "artifacts")
    end

    # Test invalid run type
    @test_throws ErrorException Postprocessor.get_run_info(parsed_args, "invalid_type")

    # Test missing job_id
    parsed_args_no_job =
        Dict("job_id_coupled" => nothing, "coupler_output_dir" => test_output_dir)
    @test_throws ErrorException Postprocessor.get_run_info(parsed_args_no_job, "coupled")
end
...
@testset "append_table_data" begin
    ...
    result = Postprocessor.append_table_data(initial_data, setup_id, job_id, artifacts_dir)
