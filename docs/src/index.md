...
- **[Available Component Models](@ref Models)**: Describes the built-in surface models (slab
  ocean, prescribed ocean, prescribed sea ice) and the more complex models provided by
  package extensions.
- **[Available Simulation Types](@ref)**: Explains each supported `mode_name`
  option (AMIP, CMIP, slabplanet variants, subseasonal) and the component models each uses.
...
- **[Performance Tips](@ref)**: CPU vs GPU guidance, reference SYPD numbers, and
  compiler flag recommendations for short vs long runs.
- **[Debugging Tips](@ref)**: Practical tips for debugging package incompatibilities,
  numerical instabilities, and software errors.
- **[CalibrationTools](@ref)**: Utilities for setting up parameter calibration experiments
  using ERA5 observational data.
- **[Contributing](@ref)**: Guidelines for contributing to ClimaCoupler.jl.
