...
function compute_soil_rhs!(dY, Y, t, p)
    ...
    κ_sat = saturated_thermal_conductivity.(θ_l, θ_i, κ_sat_unfrozen, κ_sat_frozen)
    ...
    gradf2c_heat =
        Operators.GradientF2C(top = Operators.SetValue(top_heat_flux), bottom = Operators.SetValue(btm_heat_flux))

    gradc2f_water = Operators.GradientC2F()
    gradf2c_water =
        Operators.GradientF2C(top = Operators.SetValue(top_water_flux), bottom = Operators.SetValue(btm_water_flux))
