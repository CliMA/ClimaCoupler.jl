...
function steering_summary(ekp, g_ensemble, prior, iteration)
    ...
    for b in blocks
        ...
        verdict =
            rms > T.obs_signal ? "learnable signal remains" :
            rms < T.obs_floor_lo ?
        ...
        push!(
            ...
            rpad(b.name, 6) *
            " RMS " *
            rpad(string(round(rms; digits = 2)) * "σ", 7) *
            " ▸ " *
            verdict,
        )
        ...
        if tot > 0
            ...
            flag =
                (frac < T.envelope_ok && onesided > T.onesided && rms > T.obs_signal) ?
    ...
    push!(
        lines,
        isempty(degen_msgs) ? "degen  all parameter pairs separable" :
    ...
    if length(u_hist) >= 2
        ...
        converged =
            all(d -> isfinite(d) && d < T.converged_drift, drift) &&
            !isempty(obs_done) &&
            all(obs_done)
        push!(
            lines,
