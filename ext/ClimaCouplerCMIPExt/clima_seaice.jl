...
function write_diagnosed_ice_T_sfc_to_model!(sim::ClimaSeaIceSimulation)
    ...
    # scatter_to_oc! accumulates into its own internal buffer and overwrites T_oc via
    # sync_intersection_vector!. OC cells with no intersection polygon coverage are left
    # at exactly 0 K, which after subtracting C_to_K gives top_surface_temperature =
    # -273.15°C. Fall back to the previous top_surface_temperature for those cells so
    # uncovered ice cells retain a physically meaningful temperature.
    prev_T_K = OC.interior(top_sfc_T, :, :, 1) .+ C_to_K
    T_oc_safe = ifelse.(T_oc_2d .== 0, prev_T_K, T_oc_2d)
