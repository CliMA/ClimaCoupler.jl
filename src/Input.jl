...
function argparse_settings()
    ...
    ArgParse.@add_arg_table! s begin
        ...
        default = true
        "--cov_cutoff"
        help = "Relative wet-coverage cutoff when scattering exchange-grid polygon fluxes onto the SE boundary. Nodes with DSS'd weighted coverage ≤ this value get zero flux (sliver suppression). Set to 0 to disable. [1e-3 (default)]"
        arg_type = Float64
        default = 1e-3
        "--sst_adjustment"
...
function get_coupler_args(config_dict::Dict)
    ...
    cov_cutoff = FT(config_dict["cov_cutoff"])
