...
bathymetric wet mask, so fractions and flux weights are consistent with
...
GPU-resident, allocation-free per-step application. Controlled by the
`use_intersection_grid` (default `true`; automatically disabled for column
and distributed setups) and `cov_cutoff` (default `1e-3`; set `0` to disable
sliver suppression when scattering polygon fluxes to the SE boundary)
