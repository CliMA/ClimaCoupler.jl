        ...
        NCDatasets.NCDataset(pb) do db
            NCDatasets.NCDataset(pc) do dc
                ...
                for vn in keys(da)
                    ...
                    if tdim !== nothing && length(times) > 1 && size(a, tdim) == length(times)
                        for i in 1:length(times)
                            sa = selectdim(a, tdim, i); sb = selectdim(b, tdim, i); sc = selectdim(c, tdim, i)
                            push!(out, (vn, times[i], maxabsdiff(sa, sb), maxabsdiff(sa, sc)))
                        end
                    else
...
function main()
    ...
    for comp in COMPONENTS
        ...
        for fn in sort(readdir(d))
            rows = endswith(fn, ".nc") ? nc_slices(fn, comp) :
    ...
    @printf("\n%-14s %8s %14s %14s %10s\n", "time", "fields", "worst A-B", "worst A-C", "ratio")
    # times are Float64 (NetCDF) or String iteration numbers (Oceananigans JLD2)
    sortkey(x) = x === nothing ? -Inf :
                 x isa AbstractString ? (something(tryparse(Float64, x), Inf)) :
                 Float64(x)
    for t in sort(collect(keys(bytime)), by = sortkey)
        ...
        @printf("%-14s %8d %14.5g %14.5g %10.3g\n",
                t === nothing ? "static" : string(t), length(rows), wab, wac, ratio)
    end
    ...
    flagged = [(f, v, t, ab, ac) for (f, v, t, ab, ac) in allrows
               if ac > 10 * max(ab, 0) && ac > 1e-10]
    if isempty(flagged)
        println("  none")
    else
        sort!(flagged, by = x -> -x[5])
        for (f, v, t, ab, ac) in first(flagged, 15)
            @printf("  %-46s %-14s t=%-10s A-B=%.4g  A-C=%.4g\n",
                    f, v, t === nothing ? "static" : string(t), ab, ac)
        end
