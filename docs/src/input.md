...
| `--land_fraction_source` | String | `"etopo"` | `etopo`, `era5`, `ocean` | Source for land fraction data. `etopo` uses ETOPO-derived landsea_mask artifact (binary), `era5` uses ERA5/IFS land fraction artifact (0.0 - 1.0), which includes large inland seas and lakes. `ocean` derives the mask from the Oceananigans immersed-boundary wet/dry field (requires `ocean_model: oceananigans`; recommended for CMIP). |
