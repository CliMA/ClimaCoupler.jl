...
function FluxCalculator.compute_surface_fluxes!(
    ...
    # Log extrema of individual moisture flux components
    # @info "Canopy transpiration extrema" extrema=extrema(canopy_dest.transpiration)
    # @info "Soil vapor flux liq extrema" extrema=extrema(soil_dest.vapor_flux_liq)
    # @info "Soil vapor flux ice extrema" extrema=extrema(soil_dest.vapor_flux_ice)
    # @info "Snow vapor flux extrema" extrema=extrema(snow_dest.vapor_flux)
    
    Interfacer.remap!(
        csf.temp1,
        (canopy_dest.transpiration .+
        (soil_dest.vapor_flux_liq .+ soil_dest.vapor_flux_ice) .* (1 .- p.snow.snow_cover_fraction) .+
        p.snow.snow_cover_fraction .* snow_dest.vapor_flux) .* ρ_liq, # convert from m/s to kg/m2s
    )
    @. csf.temp1 = ifelse(area_fraction == 0, zero(csf.temp1), csf.temp1)
