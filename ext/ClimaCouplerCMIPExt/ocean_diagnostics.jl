...
"""
function add_ocean_diagnostics!(ocean_sim::OceananigansSimulation;
                                output_dir = ".",
                                surface_averaging_interval = 1days,
                                field_averaging_interval = 15days,
                                checkpoint_interval = 90days,
    ...
    ocean.output_writers[:surface_averages] = OC.JLD2Writer(ocean.model, surface_outputs;
    ...
    ocean.output_writers[:field_averages] = OC.JLD2Writer(ocean.model, field_outputs;
        schedule = OC.AveragedTimeInterval(field_averaging_interval),
        filename = joinpath(output_dir, filename_prefix * "_fields"),
        file_splitting,
        overwrite_existing = true)

    ocean.output_writers[:checkpointer] = OC.Checkpointer(ocean.model;
