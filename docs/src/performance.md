...
## A note on initialization and precompilation time

Because ClimaCoupler.jl depends on a large number of packages, precompilation can take
a very long time the first time a simulation is run, or any time packages need to be
precompiled. For CMIP simulations in particular, which load additional heavy dependencies,
this can take **up to 40 minutes**. Subsequent runs reuse the precompiled cache and start much faster.

The suggestions in this page aim to reduce this initialization time, as
well as reducing runtime.
...
|---------------|--------------------------------|-------------------------------------------------------|-------|-----------------|-----------------------------|
| Slabplanet    | 1 CPU (1 core, Intel Ice Lake) | ~2.8° / h_elem = 8                                    | 0.034 | ED only         | longrun                     |
| AMIP          | 1 CPU (1 core, Intel Ice Lake) | ~3.8° / h_elem = 6                                    | 0.276 | vert. diff only | Float64 + hourly checkpoint |
| AMIP          | 1 GPU (A100)                   | ~1.4° / h_elem = 16                                   | 1.996 | ED only         | longrun                     |
| CMIP          | 1 GPU (A100)                   | ~1.4° / h_elem = 16 (atmos/land), ~1° (ocean/sea ice) | 1.017 | ED only         | longrun                     |
