...
ClimaCalibrate 0.4: the covariance realizations come from a `SampleCollection`
(`SampleBuilder.build_samples_by_times`) rather than from date ranges passed to
the estimator. One collection is built over the covariance dates; its columns
are both the noise realizations and the pool of calibration targets, selected
by index. `FT = Float64` matches the sample element type the 0.3 recipe
produced.
...
function make_data_informed_observation_vector(
    ...
    sample_collection = ClimaCalibrate.SampleBuilder.build_samples_by_times(
        vars,
        covariance_date_ranges;
        FT = Float64,
    )
    obs_vec = map(sample_date_ranges) do sample_date_range
        ...
        isnothing(idx) && error(
            "Sample date range $sample_date_range is not one of the covariance " *
            "dates; SVDplusD requires every calibration target to be one of " *
            "the covariance realizations",
        )
