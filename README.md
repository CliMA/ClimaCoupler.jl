...
julia> include("run_amip.jl")
```

Or to run from the terminal:
```julia
julia --project=experiments/ClimaEarth run_amip.jl
```
...
```julia
julia --project=experiments/ClimaEarth run_amip.jl --config_file config/ci_configs/coarse_single_ft64.jl --job_id coarse_single_ft64
```
...
The CUDA.jl and MPI.jl are required to run ClimaCoupler's AMIP experiment on GPU and in parallel (with MPI), respectively.
