...
years = 2010
sample_date_ranges = repeat([(DateTime(yr, 3, 1), DateTime(yr, 4, 1)) for yr in years], 5)

const CALIBRATE_CONFIG = CalibrateConfig(;
config_file = joinpath(pkgdir(ClimaCoupler), "experiments/calibration/coarse_amip/amip_config.yml"),
    short_names = ["rsut", ],
    minibatch_size = 1,
    n_iterations = 5,
    sample_date_ranges,
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    priors = [
        ...
        EKP.constrained_gaussian("precipitation_timescale", 1200, 300, 300, 1800)
        EKP.constrained_gaussian("entr_inv_tau", 0.0012, 0.0005, 0.0001, 0.01)
    ]
