...
function main(::Type{FT}) where {FT}
    ...
    numerics = (
        NFfirstorder = CentralNumericalFluxFirstOrder(),
        NFsecondorder = PenaltyNumFluxDiffusive(),
    ...
    atmos_uⁱⁿⁱᵗ(npt, el, x, y, z) =
        (0 * r̂(x, y, z) + 0 * ϕ̂(x, y, z) + uˡᵒⁿ(lon(x, y, z), lat(x, y, z), rad(x, y, z)) * λ̂(x, y, z))
    ...
    mA = CplModel(;
        grid = gridA,
        equations = CplMainBL(bl_propA, (CoupledPrimaryBoundary(), ExteriorBoundary()), param_set),
    ...
    mO = CplModel(;
        grid = gridO,
        equations = CplMainBL(bl_propO, (ExteriorBoundary(), CoupledSecondaryBoundary()), param_set),
        boundary_z = boundary_mask,
        nsteps = nstepsO,
        dt = Δt_ / nstepsO,
        timestepper = LSRK54CarpenterKennedy,
    ...
    cpl_solver = CplSolver(component_list = component_list, coupler = coupler, coupling_dt = Δt_, t0 = 0.0)
