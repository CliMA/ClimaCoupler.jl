...
function OceananigansSimulation(
    ...
    # Precompute polar-exclusion flux masks once (zeros ocean surface fluxes where |lat| ≥ 85° to avoid instability).
...
|lat| ≥ 85° (same band used to zero ocean surface fluxes).
...
function FieldExchanger.resolve_area_fractions!(
    ...
    if ocean_sim.ocean.model.grid.underlying_grid isa OC.LatitudeLongitudeGrid
        ...
        # Polar mask: 1 where |lat| ≥ 85° (same band used to zero ocean fluxes)
...
Returns a 2D mask (1.0 where |lat| < 85°, 0.0 elsewhere). This mask is on the ocean grid (
