...
function CouplerState(Δt_coupled)
    return CouplerState(
        ...
        Dict{Tuple{ClimaCore.Spaces.AbstractSpace, ClimaCore.Spaces.AbstractSpace}, Operators.LinearRemap}(),
        Δt_coupled,
    )
end
...
function coupler_add_field!(
    ...
    if exchange_space !== nothing
        # LinearRemap will (eventually) build ClimaCore.Operators.IdentityRemap
        # and then the following lines will be common. Can then switch to:
        # `if exchange_space === nothing; exchange_space = axes(fieldvalue); end`
...
function coupler_put!(coupler::CouplerState, fieldname::Symbol, fieldvalue, source_sim::AbstractSimulation)
    ...
    if !(axes(cplfield.data) == axes(fieldvalue))
...
"""
function coupler_get!(target_field::ClimaCore.Fields.Field, coupler::CouplerState, fieldname::Symbol)
    cplfield = coupler.coupled_fields[fieldname]
...
function coupler_get_map(
    coupler,
    target_space::ClimaCore.Spaces.AbstractSpace,
    source_space::ClimaCore.Spaces.AbstractSpace,
)
